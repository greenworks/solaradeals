<?php
/**
 * @version 1.5.3 $Id: admin.rstestimonials.php
...
<table width="100%" cellpadding="0" cellspacing="0"><tr><td><h1>Manage Testimonials</h1></td><td align="right"><table cellpadding="0" cellspacing="0"><tr><td style="padding:0 10px;"><a href="javascript:void(0);" title="Delete Selected Testimonials" onclick="return delTesti();"><img src="images/delete_f2.png" border="0" alt="Delete Selected Testimonials" /></a></td><td style="padding:0 10px;"><a href="index.php?option=<?php echo $_REQUEST['option']; ?>&task=<?php echo $_REQUEST['task']; ?>&action=add&page=<?php echo $_REQUEST['page']; ?>&limit=<?php echo $_REQUEST['limit']; ?>" title="Add New Testimonial"><img src="images/new_f2.png" border="0" alt="Add" /></a></td></tr></table></td></tr></table>
</div>
<div>&nbsp;</div>
<div id="editcell">
		...
		<tbody>
			...
			<tr class="row<?php echo ($cnt%2); ?>">
                ...
                <td valign="top" align="center">
                	<?php
					$testi_pic_file = '';
					if(file_exists('../components/com_rsmonials/images/'.$item->id.'.gif')) {
						$testi_pic_file = '../components/com_rsmonials/images/'.$item->id.'.gif';
					} else if(file_exists('../components/com_rsmonials/images/'.$item->id.'.png')) {
						$testi_pic_file = '../components/com_rsmonials/images/'.$item->id.'.png';
					} else if(file_exists('../components/com_rsmonials/images/'.$item->id.'.jpg')) {
						$testi_pic_file = '../components/com_rsmonials/images/'.$item->id.'.jpg';
					} else if(file_exists('../components/com_rsmonials/images/'.$item->id.'.jpeg')) {
						$testi_pic_file = '../components/com_rsmonials/images/'.$item->id.'.jpeg';
					}
					if($testi_pic_file != '') {
		?>
                ...
                <?php } ?>
                <td valign="top"><?php if(fetchParam('show_single_name_field') == 'true') { echo $item->fname; } else { echo $item->fname.' '.$item->lname; } ?><?php if($item->status=='2') { ?><img src="images/new_f2.png" border="0" alt="New Testimonials" height="16" width="16" /><?php } ?></td>
				...
				<td align="center" valign="top"><a href="index.php?option=<?php echo $_REQUEST['option']; ?>&task=<?php echo $_REQUEST['task']; ?>&action=edit&id=<?php echo $item->id; ?>&page=<?php echo $_REQUEST['page']; ?>&limit=<?php echo $_REQUEST['limit']; ?>" title="Edit Item"><img src="images/edit_f2.png" border="0" alt="Edit" width="20" /></a></td>
				<td align="center" valign="top"><a href="index.php?option=<?php echo $_REQUEST['option']; ?>&task=<?php echo $_REQUEST['task']; ?>&action=delete&id=<?php echo $item->id; ?>&page=<?php echo $_REQUEST['page']; ?>&limit=<?php echo $_REQUEST['limit']; ?>" title="Delete Item" onclick="javascript:if(confirm('Delete testimonial #<?php echo $item->id; ?>. Are you sure?')){return true;}else{return false;}"><img src="images/delete_f2.png" border="0" alt="Delete" width="20" /></a></td>
			</tr>
			<?php
				$cnt++;
			}
...
<fieldset class="adminform">
    ...
    <tr>
        ...
        <?php if($row->id > 0) {
			$testi_pic_file = '';
			if(file_exists('../components/com_rsmonials/images/'.$row->id.'.gif')) {
				$testi_pic_file = '../components/com_rsmonials/images/'.$row->id.'.gif';
			} else if(file_exists('../components/com_rsmonials/images/'.$row->id.'.png')) {
				$testi_pic_file = '../components/com_rsmonials/images/'.$row->id.'.png';
			} else if(file_exists('../components/com_rsmonials/images/'.$row->id.'.jpg')) {
				$testi_pic_file = '../components/com_rsmonials/images/'.$row->id.'.jpg';
			} else if(file_exists('../components/com_rsmonials/images/'.$row->id.'.jpeg')) {
				$testi_pic_file = '../components/com_rsmonials/images/'.$row->id.'.jpeg';
			}
			if($testi_pic_file != '') {
		?>
...
function save() {
	...
	if($_POST['testi_pic_del'] == 'true') {
		$testi_pic_file = '';
		if(file_exists(JPATH_ROOT.DS.'components'.DS.'com_rsmonials'.DS.'images'.DS.$_POST['id'].'.gif')) {
			$testi_pic_file = JPATH_ROOT.DS.'components'.DS.'com_rsmonials'.DS.'images'.DS.$_POST['id'].'.gif';
		} else if(file_exists(JPATH_ROOT.DS.'components'.DS.'com_rsmonials'.DS.'images'.DS.$_POST['id'].'.png')) {
			$testi_pic_file = JPATH_ROOT.DS.'components'.DS.'com_rsmonials'.DS.'images'.DS.$_POST['id'].'.png';
		} else if(file_exists(JPATH_ROOT.DS.'components'.DS.'com_rsmonials'.DS.'images'.DS.$_POST['id'].'.jpg')) {
			$testi_pic_file = JPATH_ROOT.DS.'components'.DS.'com_rsmonials'.DS.'images'.DS.$_POST['id'].'.jpg';
		} else if(file_exists(JPATH_ROOT.DS.'components'.DS.'com_rsmonials'.DS.'images'.DS.$_POST['id'].'.jpeg')) {
			$testi_pic_file = JPATH_ROOT.DS.'components'.DS.'com_rsmonials'.DS.'images'.DS.$_POST['id'].'.jpeg';
		}
		if($testi_pic_file != '') {
	...
	if(is_uploaded_file($_FILES['testi_pic']['tmp_name'])) {
		...
		if(($img_settings[2] != 1) && ($img_settings[2] != 2) && ($img_settings[2] != 3)) {
			...
			$upload_path = JPATH_ROOT.DS.'components'.DS.'com_rsmonials'.DS.'images'.DS.$new_id.'.';
			if($img_settings[2] == 1) {
				$upload_path .= 'gif';
