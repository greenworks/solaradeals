<?php
/**
 * @version 1.5.3 $Id: rsmonials.php
...
<?php if ( $params->def( 'show_page_title', 1 ) ) { ?>
	<div class="componentheading<?php echo $params->get( 'pageclass_sfx' ); ?>">
		<?php echo stripHTML($params->get('page_title')); ?>
	</div>
<?php } else { ?>
	<div class="componentheading"><?php echo JText::_('RSM_PAGE_TITLE'); ?></div>
<?php } ?>
<?php
...
?>
<?php if(fetchParam('show_form') != 'false') { ?><p class="RSWS_submit_link"><a href="<?php echo JRoute::_($_SERVER['REQUEST_URI'].'#submitform'); ?>"><?php echo JText::_('RSM_TXT_SUBMIT_A_TESTI'); ?></a></p><?php } ?>
<?php
}
...
<?php } ?>
<p>&nbsp;</p>
<div style="text-align:center;"><?php echo base64_decode('UG93ZXJlZCBieQ==').' <a href="'.base64_decode('aHR0cDovL3d3dy5yc3dlYnNvbHMuY29t').'" target="_blank">'.base64_decode('UlMgV2ViIFNvbHV0aW9ucw==').'</a>'; ?></div>
<p>&nbsp;</p>
<?php
}
		...
		foreach($items as $item) {
			...
			<div class="RSWS_testi_block">
				...
				if(fetchParam('show_image') == 'true') {
					$testi_pic_file = '';
					if(file_exists(JPATH_ROOT.DS.'components'.DS.'com_rsmonials'.DS.'images'.DS.$item->id.'.gif')) {
						$testi_pic_file = '<img class="RSWS_testi_img" src="'.JURI::root().'components/com_rsmonials/images/'.$item->id.'.gif" style="width:'.$rsws_imgmax.'px;" />';
					} else if(file_exists(JPATH_ROOT.DS.'components'.DS.'com_rsmonials'.DS.'images'.DS.$item->id.'.png')) {
						$testi_pic_file = '<img class="RSWS_testi_img" src="'.JURI::root().'components/com_rsmonials/images/'.$item->id.'.png" style="width:'.$rsws_imgmax.'px;" />';
					} else if(file_exists(JPATH_ROOT.DS.'components'.DS.'com_rsmonials'.DS.'images'.DS.$item->id.'.jpg')) {
						$testi_pic_file = '<img class="RSWS_testi_img" src="'.JURI::root().'components/com_rsmonials/images/'.$item->id.'.jpg" style="width:'.$rsws_imgmax.'px;" />';
					} else if(file_exists(JPATH_ROOT.DS.'components'.DS.'com_rsmonials'.DS.'images'.DS.$item->id.'.jpeg')) {
						$testi_pic_file = '<img class="RSWS_testi_img" src="'.JURI::root().'components/com_rsmonials/images/'.$item->id.'.jpeg" style="width:'.$rsws_imgmax.'px;" />';
					} else {
...
function submit() {
	...
	if($isfalse == false) {
		...
		if(is_uploaded_file($_FILES['testi_pic']['tmp_name'])) {
			...
			$upload_path = JPATH_ROOT.DS.'components'.DS.'com_rsmonials'.DS.'images'.DS.$new_t_id.'.';
			$img_settings = getimagesize($_FILES['testi_pic']['tmp_name']);
			if($img_settings[2] == 1) {
				$upload_path .= 'gif';
